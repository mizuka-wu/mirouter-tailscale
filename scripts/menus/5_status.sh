...
show_status() {
    ...
    # 显示实际运行的二进制版本 (不是配置里的版本)
    if [ -x "$BIN_TS" ]; then
        real_version=$("$BIN_TS" version 2>/dev/null | head -1)
        content_line "版本:    $real_version"
    else
        content_line "版本:    \033[33m未安装\033[0m (配置: v${ts_version})"
    fi
